...
export const POS = () => {
  ...
  // ─── Realtime Subscriptions for POS ───
  useEffect(() => {
    if (!tenant?.id) return;

    const channel = supabase
      .channel(`admin-pos-realtime-${tenant.id}`)
      .on(
        'postgres_changes',
  ...
  const availableTables = tables.filter((t: any) => (t.status === 'AVAILABLE' && (t.isActive ?? true)) || t.id === selectedTable);
